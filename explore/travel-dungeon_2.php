<?php
	...
	if($mark = $hero->Mark($explore["floor"]))
	{
		...
		$floorInfluence = floor($explore["floor"] / 11);
		$minLevel = $floorInfluence * 2 + 15;
		$maxLevel = $minLevel + 3;
		
		$minLevel = max($hero->Level(), $minLevel);
		$minLevel = min(40, $minLevel);
		$maxLevel = min(40, $maxLevel);
	...
	# BATTLE #
		...
		$minLevel = max(15, $minLevel);
		$minLevel = min(40, $minLevel);
		$maxLevel = min(40, $maxLevel);
		
		$monster["level"] = rand($minLevel, $maxLevel);
		$monster["tier"] = rand(1, 4);
